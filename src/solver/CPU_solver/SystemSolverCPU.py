...
class SolverCPU():
    def __init__(self, init_params: dict) -> None:
        ...
        self.q_in = bc['q_in']
        self.q_out = bc['q_out']
        self.c_in_times = np.asarray(bc['c_in_times'])
        self.c_in_arr = np.asarray(bc['c_in_arr'])
    ...
    def pressure_and_velocities(self, c: np.ndarray, p0: np.ndarray):
        Lambda = -self.w**2 / (12 * self.mu(c))
        LambdaX = self.Harmonic(Lambda.T).T
        LambdaY = self.Harmonic(Lambda)
    ...
    def make_step(self, Q: np.ndarray, P: np.ndarray, Vx: np.ndarray, Vy: np.ndarray, current_time: float):
    ...
    def solve(self, tmax: float = 1.0) -> None:
    ...
    def rk2_cycle(self, Q: np.ndarray, P: np.ndarray, Vx: np.ndarray, Vy: np.ndarray, c_in_value: float, dt: float):
        q_left = np.where(self.q_in == 0, Q[:,0], c_in_value*self.w[:,0])
        q_RK1 = self.TransportSolver.RK(Q, Q, dt, Vx, Vy, q_left, 1)
        if self.beta == 0 and (self.g==0 or self.rho1==self.rho2):
    ...
    def rk3_cycle(self, Q: np.ndarray, P: np.ndarray, Vx: np.ndarray, Vy: np.ndarray, c_in_value: float, dt: float):
        q_left = np.where(self.q_in == 0, Q[:,0], c_in_value*self.w[:,0])
